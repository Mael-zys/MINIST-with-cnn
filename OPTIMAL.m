...
%         end
       ...
       %%  梯度（具体公式原理在报告里）
        ...
        %w1,b1梯度
         ...
         %计算改变量
        delta_ws=mom*delta_ws+alpha*(ws_grad/minibatch+weight_decay*ws);
        delta_bs=mom*delta_bs+alpha*(bs_grad/minibatch);
        delta_w2=mom*delta_w2+alpha*(w2_grad/minibatch+weight_decay*w2);
        delta_b2=mom*delta_b2+alpha*(b2_grad/minibatch);
        delta_w1=mom*delta_w1+alpha*(w1_grad/minibatch+weight_decay*w1);
        delta_b1=mom*delta_b1+alpha*(b1_grad/minibatch);
